from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import soundfile as sf
from scipy import signal

from src.config import DatasetConfig, RAW_DIR, OUTPUT_DIR, SAMPLE_TO_FILE


def fig_4_2_spectrum_sci() -> None:
    """SCI/EI style spectrum comparison with smoothing and subplots.

    - Use Welch PSD for smoothing.
    - Three vertical subplots: raw / filtered / pre-emphasized.
    - Frequency 0–20 kHz, Y-lim tightened to around [-120, max].
    - English labels and high-contrast, print-friendly colors.
    """
    cfg = DatasetConfig()
    sample_id = "Sample1"
    wav_path = RAW_DIR / SAMPLE_TO_FILE[sample_id]

    raw, sr = sf.read(wav_path)
    raw = raw.astype(np.float64)

    # middle 0.5 s segment to avoid transients
    center = len(raw) // 2
    half = int(0.25 * sr)
    seg = raw[center - half : center + half]

    # normalization
    if np.max(np.abs(seg)) > 0:
        norm = seg / np.max(np.abs(seg))
    else:
        norm = seg

    # preprocessing steps
    sos = signal.butter(
        4,
        [cfg.bandpass_low, cfg.bandpass_high],
        btype="bandpass",
        fs=cfg.sample_rate,
        output="sos",
    )
    bp = signal.sosfilt(sos, norm)
    b, a = signal.iirnotch(cfg.notch_freq, 30, fs=cfg.sample_rate)
    bp_notch = signal.filtfilt(b, a, bp)
    pre = signal.lfilter([1.0, -cfg.pre_emphasis], [1], bp_notch)

    def welch_db(sig: np.ndarray):
        f, Pxx = signal.welch(sig, fs=sr, window="hann", nperseg=2048, noverlap=1024)
        Pxx_db = 10 * np.log10(Pxx + 1e-15)
        return f, Pxx_db

    f_raw, psd_raw = welch_db(seg)
    _, psd_bp = welch_db(bp_notch)
    _, psd_pre = welch_db(pre)

    # common frequency mask 0–20 kHz
    mask = (f_raw >= 0) & (f_raw <= 20000)
    f_khz = f_raw[mask] / 1000.0
    psd_raw = psd_raw[mask]
    psd_bp = psd_bp[mask]
    psd_pre = psd_pre[mask]

    # determine y-limits: tighten to [-120, max+5] but not below -120
    peak = max(psd_raw.max(), psd_bp.max(), psd_pre.max())
    y_top = peak + 5
    y_bottom = max(peak - 80, -120)

    fig, axes = plt.subplots(3, 1, figsize=(6.5, 7), sharex=True)
    plt.rcParams["font.sans-serif"] = ["Arial"]

    configs = [
        (psd_raw, "Raw spectrum", "#1f77b4"),
        (psd_bp, "After band-pass + notch", "#d62728"),
        (psd_pre, "After pre-emphasis", "#4d4d4d"),
    ]

    for ax, (psd, title, color) in zip(axes, configs):
        ax.plot(f_khz, psd, color=color, lw=0.9)
        ax.set_xlim(0, 20)
        ax.set_ylim(y_bottom, y_top)
        ax.set_ylabel("Magnitude (dB)", fontsize=11)
        ax.set_title(title, fontsize=11)
        ax.grid(True, linestyle="--", alpha=0.3)
        ax.tick_params(labelsize=9)

    axes[-1].set_xlabel("Frequency (kHz)", fontsize=11)

    fig.suptitle(
        "Fig. 4-2 Spectrum before and after preprocessing (Welch PSD, 0.5 s segment)",
        fontsize=12,
    )
    fig.tight_layout(rect=[0, 0.05, 1, 0.95])
    out_path = OUTPUT_DIR / "fig_4-2_spectrum_sci.png"
    out_path.parent.mkdir(exist_ok=True)
    fig.savefig(out_path, dpi=300)
    plt.close(fig)


if __name__ == "__main__":
    fig_4_2_spectrum_sci()
