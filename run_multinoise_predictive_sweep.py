"""
Sweep predictive-MDS fusion configs to highlight perceptual layer contribution.

Configs varied:
- alpha in {3, 5, 8}
- perception regressor:
    * SVR_default: C=5, gamma=0.05, eps=0.05
    * SVR_strong:  C=10, gamma=0.1, eps=0.01
    * MLP:         hidden_layer_sizes=(64,), relu

Baseline (no MDS) is included once per run for reference.

Training noise: white/pink/factory × clean/10/8/5/0.5 dB.
Testing noise: same grid.

Outputs:
  outputs/metrics_summary_multinoise_sweep.csv
  outputs/metrics_rounds_multinoise_sweep.csv
"""
...
# Use more rounds for stability; adjust if runtime is too long
N_ROUNDS = 2
...
# Config list: (name, alpha, regressor_builder)
def make_svr_default():
    return MultiOutputRegressor(
        Pipeline(
            [("scaler", StandardScaler()), ("svr", SVR(kernel="rbf", C=5.0, gamma=0.05, epsilon=0.05))]
        )
    )
...
def make_svr_strong():
    return MultiOutputRegressor(
        Pipeline(
            [("scaler", StandardScaler()), ("svr", SVR(kernel="rbf", C=10.0, gamma=0.1, epsilon=0.01))]
        )
    )
    ...
    return MultiOutputRegressor(
        Pipeline(
            [
                ...
                ("mlp", MLPRegressor(hidden_layer_sizes=hidden, activation="relu", max_iter=800, random_state=0)),
            ]
        )
    )
...
SWEEP_CONFIGS = [
    ("alpha8_svr_strong", 8.0, make_svr_strong),
    ("alpha10_svr_strong", 10.0, make_svr_strong),
    ("alpha8_mlp128_64", 8.0, lambda: make_mlp((128, 64))),
    ("alpha10_mlp128_64", 10.0, lambda: make_mlp((128, 64))),
]
...
def _feature_matrix_clean_only(segments: Sequence[Segment], cfg: DatasetConfig) -> Tuple[np.ndarray, np.ndarray]:
    feats: List[np.ndarray] = []
    groups: List[str] = []
    for seg in segments:
        ...
        for snr_db in NOISE_LEVELS_TEST:
            ...
            for seg in segments:
                ...
                if include_pred_mds:
                    ...
                    base = np.concatenate([base, alpha * pred.ravel()])
                feats.append(base)
                labels.append(seg.label)
...
def run_round(cfg_name: str, alpha: float, reg_builder, round_idx: int, cfg: DatasetConfig, segments, mds_map, factory_noises):
    ...
    # baseline trained once per round
    clf_baseline = _train_classifier(X_train, y_train, groups)

    # perception regressor on clean only
    X_reg, groups_reg = _feature_matrix_clean_only(train_segments, cfg)
    reg = reg_builder()
    y_mds = np.stack([mds_map[g] for g in groups_reg], axis=0)
    reg.fit(X_reg, y_mds)

    mds_pred_train = reg.predict(X_train)
    X_fused = np.hstack([X_train, alpha * mds_pred_train])
    clf_fusion = _train_classifier(X_fused, y_train, groups)

    rows = []
    rows.extend(
        [
            {"config": cfg_name, "model": "baseline", **r}
            for r in _evaluate(clf_baseline, reg, alpha, test_segments, cfg, factory_noises, include_pred_mds=False)
        ]
    )
    rows.extend(
        [
            {"config": cfg_name, "model": "fusion_pred_mds", **r}
            for r in _evaluate(clf_fusion, reg, alpha, test_segments, cfg, factory_noises, include_pred_mds=True)
        ]
    )
    for r in rows:
        r["round"] = round_idx
...
def main():
    cfg = DatasetConfig(max_segments_per_sample=25)
    ensure_output_dirs()
    ...
    for name, alpha, builder in SWEEP_CONFIGS:
        for r in range(N_ROUNDS):
            all_rows.extend(run_round(name, alpha, builder, r, cfg, segments, mds_map, factory_noises))

    metrics_df = pd.DataFrame(all_rows)
    metrics_df.to_csv(OUTPUT_DIR / "metrics_rounds_multinoise_sweep.csv", index=False)
    summary = metrics_df.groupby(["config", "model", "noise_type", "snr"]).agg(
        accuracy_mean=("accuracy", "mean"),
        accuracy_std=("accuracy", "std"),
        macro_f1_mean=("macro_f1", "mean"),
        macro_f1_std=("macro_f1", "std"),
    ).reset_index()
    summary.to_csv(OUTPUT_DIR / "metrics_summary_multinoise_sweep.csv", index=False)
    with open(OUTPUT_DIR / "summary_multinoise_sweep.json", "w", encoding="utf-8") as f:
        json.dump(summary.to_dict(orient="records"), f, indent=2)
    print("Wrote sweep outputs to", OUTPUT_DIR)
